...
int main(int argc, char *argv[]) 
{
    ...
    int newTriangle[TRIANGLE_SIZE][TRIANGLE_SIZE] =
    {
    ...
    int resultTriangle[TRIANGLE_SIZE][TRIANGLE_SIZE] = {};
    int maxRows = TRIANGLE_SIZE-1;
    ...
    for (int i = maxRows; i >= 0; i--)
    {
        for (int j = 0; j <= i; j++)
        {
            if (i == maxRows)
                ...
                resultTriangle[i][j] = newTriangle[i][j] + std::max (resultTriangle[i+1][j], resultTriangle[i+1][j+1]);
        }
    }
    
    std::cout << "The final sum is: " << resultTriangle[0][0];
}
