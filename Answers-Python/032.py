...
#
def integer_from_array(input_array, start_index, stop_index):
    output = 0
    count = 0

    for i in range(start_index, stop_index + 1):
        output += input_array[i] * pow(10, count)
        count += 1
...
while next_permutation(digits):
    ...
    for i in range(0, 6 + 1):
        multiplicand = integer_from_array(digits, 0, i);

        # Multiplier loop. May be up to seven digits.
        for j in range(i + 1, 7 + 1):
            multiplier = integer_from_array(digits, i + 1, j);

            # Generate and test the product
            product = multiplier * multiplicand

            if product == integer_from_array(digits, j + 1, 8):
                values.add(product)
