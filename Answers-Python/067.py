...
result_triangle = [[0] * TRIANGLE_SIZE for i in range(TRIANGLE_SIZE)]
...
for i in range(maxRows, 0-1, -1):
    for j in range(0, i+1):
        if (i == maxRows):
            result_triangle[i][j] = new_triangle[i][j]
        else:
            print (i,j)
            result_triangle[i][j] = new_triangle[i][j] + max(result_triangle[i + 1][j], result_triangle[i + 1][j + 1])

print ("The final sum is: ", result_triangle[0][0])
