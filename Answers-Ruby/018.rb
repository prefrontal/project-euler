...
resultTriangle = Array.new(TRIANGLE_SIZE, Array.new(TRIANGLE_SIZE))
...
for i in maxRows.downto(0)
	for j in 0.upto(i)
		if (i == maxRows)
			resultTriangle[i][j] = newTriangle[i][j];
		else
			resultTriangle[i][j] = newTriangle[i][j] + [resultTriangle[i+1][j], resultTriangle[i+1][j+1]].max
		end
	end
end

puts "The final sum is: #{resultTriangle[0][0]}"
