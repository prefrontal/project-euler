...
a = Array.new(DIGIT_TARGET,0)
b = Array.new(DIGIT_TARGET,0)
c = Array.new(DIGIT_TARGET,0)
...
loop do
	...
	for i in 0...DIGIT_TARGET
		temp = b[i] + c[i] + remainder
		a[i] = temp % 10
		remainder = (temp / 10).to_i
	end
	
	# Update the last two values
	for j in 0...DIGIT_TARGET
	...
	for k in (DIGIT_TARGET-1).downto(0)
	...
	break if (digitCount == DIGIT_TARGET)
end

puts "The term with #{DIGIT_TARGET} digits is: #{termCount}"