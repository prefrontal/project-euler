...
def FindPentagonalNumbers (maximum)
	...
	for i in 1..maximum
		numbers.push (CalculatePentagonal(i))
	end
...
for i in 0...MAXIMUM_VALUE # Outer loop
	for j in i...MAXIMUM_VALUE # Inner loop
		sum = numbers[i] + numbers[j];
		difference = numbers[j] - numbers[i];
					
		if (IsPentagonal(sum) && IsPentagonal(difference))
			finalAnswer = (CalculatePentagonal(j+1) - CalculatePentagonal(i+1)).abs
		end
	end
end

puts "The final answer is: #{finalAnswer}"
