// Project Euler Problem 11, Swift
//
...
var grid = Array(count: 20, repeatedValue: Array(count: 20, repeatedValue: 0))
...
grid[5] =  [24, 47, 32, 60, 99, 03, 45,  2, 44, 75, 33, 53, 78, 36, 84, 20, 35, 17, 12, 50]
grid[6] =  [32, 98, 81, 28, 64, 23, 67, 10, 26, 38, 40, 67, 59, 54, 70, 66, 18, 38, 64, 70]
grid[7] =  [67, 26, 20, 68, 02, 62, 12, 20, 95, 63, 94, 39, 63,  8, 40, 91, 66, 49, 94, 21]
...
grid[13] = [19, 80, 81, 68, 05, 94, 47, 69, 28, 73, 92, 13, 86, 52, 17, 77,  4, 89, 55, 40]
...
grid[18] = [20, 73, 35, 29, 78, 31, 90,  1, 74, 31, 49, 71, 48, 86, 81, 16, 23, 57, 05, 54]
grid[19] = [ 1, 70, 54, 71, 83, 51, 54, 69, 16, 92, 33, 48, 61, 43, 52,  1, 89, 19, 67, 48]

let numWindows = 20-WINDOW_LENGTH+1;
var maxProduct = 0;
...
for (var i = 0; i < 20; i++)
{
	...
	{
		var product = grid[i][k] * grid[i][k+1] * grid[i][k+2] * grid[i][k+3]
...
for (var j = 0; j < 20; j++)
{
	for (var k = 0; k < numWindows; k++)
	{
		var product = grid[k][j] * grid[k+1][j] * grid[k+2][j] * grid[k+3][j]
...
// Check diagonal elements (northwest-southeast)
...
// Check diagonal elements (southwest-northeast)
for (var i = 4; i < 20; i++)
{
	for (var j = 0; j < numWindows; j++)
	{
...
print ("The maximum product is: \(maxProduct)")
